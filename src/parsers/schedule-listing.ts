...
export function parseScheduleListing(html: string) {
  ...
  const dataDisplayTable = document.querySelector(
    '.pagebodydiv > .datadisplaytable'
  );
  if (!dataDisplayTable) {
    return [];
  }
  ...
  const result = sectionGroups
    .map(group => {
      ...
      const crn =
        (anchorWithCrn && formDecode(anchorWithCrn.href || '').crn_in) || '';
...
function parseSectionElement(body: HTMLTableRowElement | undefined | null) {
  ...
  const section: SectionTableParseResult = {
    ...
    scheduleType: (result.scheduleType || []).map(type =>
